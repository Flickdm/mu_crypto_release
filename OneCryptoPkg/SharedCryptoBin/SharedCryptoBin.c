...
{
  SHARED_CRYPTO_PROTOCOL *CryptoProtocol = (SHARED_CRYPTO_PROTOCOL *)Crypto;
  
  //
  // Initialize BaseCryptLib first
  //
  BaseCryptLibInitialize();
  
  if (CryptoProtocol == NULL) {
    DEBUG ((DEBUG_ERROR, "SharedCryptoBin: CryptoInit called with NULL protocol\n"));
    return;
  }
  ...
  DEBUG ((DEBUG_INFO, "SharedCryptoBin: CryptoInit completed - comprehensive protocol populated\n"));
}
