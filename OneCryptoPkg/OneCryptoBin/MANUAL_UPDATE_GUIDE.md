...
- Update version numbers (VERSION_MAJOR, VERSION_MINOR, VERSION_REVISION)
...
1. **Define in OneCryptoDefinitions.h**
   ...
   - Add `@since` version tag (e.g., `@since 1.0.0`)
...
```c
// ------------------------------------------------------------------------------
// ORIGINALLY AUTOGENERATED BY CreateCryptoProtocol.py
// NOW MAINTAINED MANUALLY - See MANUAL_UPDATE_GUIDE.md
...
To reference the original generation logic:
```bash
git show backup/codegen-scripts:OneCryptoPkg/OneCryptoBin/Scripts/CreateCryptoProtocol.py
```

## Questions?

If you need to reference how the automated generation worked, see the backup branch or review the git history before the scripts were removed.
