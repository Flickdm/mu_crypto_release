...
# Path to the OneCryptoDefinitions.h file
ONE_CRYPTO_DEFINITIONS = os.path.join(LIBRARY_INCLUDE_PATH, "OneCryptoDefinitions.h")

# Include headers that are required in each generated file
INCLUDE_HEADERS = [
    ONE_CRYPTO_DEFINITIONS
]
...
def cli() -> dict:
    ...
    parser.add_argument(
        "--output-protocol", help="The output file to write the transformed protocol header file to", default=os.path.join(PROTOCOL_INCLUDE_PATH, "OneCryptoProtocol.h"))
...
def create_protocol_structure(functions: dict, indent=" " * 2) -> str:
    ...
    structure = ""
    structure += "\n/**\n"
    structure += f"{indent}@struct _ONE_CRYPTO_PROTOCOL\n"
    structure += f"{indent}@brief This structure defines the protocol for shared cryptographic operations.\n\n"
    structure += f"{indent}The _ONE_CRYPTO_PROTOCOL structure provides a standardized interface for\n"
    ...
    structure += f"{indent}@ingroup OneCryptoProtocol\n"
    structure += "**/\n"

    #
    # Begin the structure
    #
    structure += "typedef struct _ONE_CRYPTO_PROTOCOL\n{\n"
    ...
    structure += "} ONE_CRYPTO_PROTOCOL;\n\n"

    return structure


def generate_autogenerated_file_comment(version, description="This file provides Crypto services") -> str:
    """
    Generates a UEFI-style file header with copyright and license information.
    
    Args:
        version: Tuple of (major, minor, revision) version numbers
    ...
    lines += "  DO NOT MODIFY\n"
    lines += "\n"
    lines += "  Copyright (C) Microsoft Corporation. All rights reserved.\n"
    lines += "  SPDX-License-Identifier: BSD-2-Clause-Patent\n"
    lines += "**/\n"
    return lines
...
def generate_protocol_header(function_info: FunctionInfo, output_file: str):
    ...
    template_file = os.path.join(TEMPLATES_PATH, "OneCryptoProtocol.template.h")
    ...
    with open(template_file, 'r') as template:
        ...
        logger.info(f"Read template file {template_file}")

    # Build the auto-generated content
    auto_generated_content = ""
    auto_generated_content += generate_autogenerated_file_comment(function_info.version)
    auto_generated_content += f"#ifndef {guard_statement}\n"
    auto_generated_content += f"#define {guard_statement}\n\n"
    auto_generated_content += "#include <Uefi.h>\n"
    ...
    for include in INCLUDE_HEADERS:
        ...
        auto_generated_content += f"#include <Library/{file_name}>\n\n"

    auto_generated_content += create_divider()
    auto_generated_content += f"// Protocol version: {major}.{minor}.{revision}\n"
    auto_generated_content += create_divider()
    auto_generated_content += f"#define VERSION_MAJOR     {major}ULL\n"
    auto_generated_content += f"#define VERSION_MINOR     {minor}ULL\n"
    auto_generated_content += f"#define VERSION_REVISION  {revision}ULL\n"

    auto_generated_content += "\n"
    auto_generated_content += create_divider()
    auto_generated_content += "// Typedef Declarations\n"
    auto_generated_content += create_divider()
    auto_generated_content += typedefs
    auto_generated_content += create_divider()
    auto_generated_content += "// Protocol\n"
    auto_generated_content += create_divider()
    auto_generated_content += protocol_structure

    auto_generated_content += f"#endif // {guard_statement}\n"

    # Replace the template replacement tag with the actual content
    output_content = template_content.replace(TEMPLATE_REPLACEMENT_TAG, auto_generated_content)

    # Write the final output
    with open(output_file, 'w') as f:
        f.write(output_content)
        logger.info(f"Wrote protocol header to {output_file}")
...
def generate_library_headers(function_info: FunctionInfo):
    ...
    for group in grouped_functions:
        ...
        template_file = os.path.join(TEMPLATES_PATH, f"{basename.replace('.h', '.template.h')}")
        ...
        if os.path.exists(template_file):
            ...
            auto_generated_content += generate_autogenerated_file_comment(function_info.version, f"Library interface for {group} cryptographic operations")
            auto_generated_content += f"#ifndef {guard_statement}\n"
            auto_generated_content += f"#define {guard_statement}\n\n"
            auto_generated_content += "#include <Uefi.h>\n"
            ...
            auto_generated_content += create_divider()
            auto_generated_content += f"// Library for {group}\n"
            auto_generated_content += create_divider()
            auto_generated_content += "\n"

            for function_name in grouped_functions[group]:
                function = function_info.functions[function_name]
                auto_generated_content += f"{function.comment}\n"
                auto_generated_content += f"{function.return_type}\n"
                auto_generated_content += f"{function.calling_convention}\n"
                auto_generated_content += f"{function_name} (\n"
                auto_generated_content += f"{function.params}\n);\n\n"
            ...
            with open(library_file, 'w') as f:
                f.write(output_content)
                logger.info(f"Library file written to {library_file} using template")
        else:
            # Fall back to direct generation if no template exists
            logger.warning(f"Template file not found: {template_file}, generating directly")
            
            guard_statement = f"{convert_to_underscores(os.path.basename(basename).split('.')[0])}_"
            with open(library_file, 'w') as f:
                description = f"Library interface for {group} cryptographic operations"
                f.write(generate_autogenerated_file_comment(function_info.version, description))
