...
{
  EFI_STATUS                          Status;
  ONE_CRYPTO_MM_CONSTRUCTOR_PROTOCOL  *ConstructorProtocol;
  EFI_HANDLE                          ProtocolHandle = NULL;

  DEBUG ((DEBUG_INFO, "OneCryptoLoaderMm: Entry point called.\n"));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Failed to locate OneCrypto private protocol: %r\n", Status));
    goto Exit;
  }

  if (ConstructorProtocol->Signature != ONE_CRYPTO_MM_CONSTRUCTOR_PROTOCOL_SIGNATURE) {
    DEBUG ((DEBUG_ERROR, "OneCrypto private protocol signature is invalid: %x\n", ConstructorProtocol->Signature));
    return EFI_UNSUPPORTED;
  }

  DEBUG ((DEBUG_INFO, "OneCrypto private protocol found: %g\n", &gOneCryptoPrivateProtocolGuid));

  //
  // Ensure that the crypto entry function is not NULL
  //
  ASSERT (ConstructorProtocol->Entry != NULL);
  ...
  //
  Status = ConstructorProtocol->Entry (mOneCryptoDepends, &OneCryptoProtocol);
  if (EFI_ERROR (Status)) {
  ...
  DEBUG ((DEBUG_INFO, "OneCrypto Protocol installed successfully.\n"));
