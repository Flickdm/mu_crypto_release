  ...
  )
{
  ...
  DEBUG ((DEBUG_ERROR, "OneCryptoLoaderDxe: About to call crypto entry at %p\n", Entry));

  //
  // Call library constructor to generate the protocol
  // Constructor will allocate memory and assign it to mOneCryptoProtocol
  // Using VOID** to be agnostic about the actual protocol structure
  //
  Status = Entry (mOneCryptoDepends, &mOneCryptoProtocol);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Failed to call crypto entry: %r\n", Status));
    goto Exit;
  }

  DEBUG ((DEBUG_ERROR, "OneCryptoLoaderDxe: Crypto entry completed successfully\n"));
  ...
  DEBUG ((DEBUG_ERROR, "OneCrypto Protocol installed successfully.\n"));
