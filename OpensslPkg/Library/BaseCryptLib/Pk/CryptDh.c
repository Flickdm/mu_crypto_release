...
{
  return (VOID *)AllocateZeroPool (sizeof (DH_PKEY_CTX));
}
  ...
  )
{
  ...
  //
  // Convert prime bytes to BIGNUM and build generator BIGNUM.
  //
  ...
  )
{
  ...
  if ((Size > 0) && (*PublicKeySize < (UINTN)Size)) {
    *PublicKeySize = Size;
    //
    // Keep Ctx->Pkey; caller may retry with a larger buffer.
    //
    BnPubKey = NULL;  // freed below; set NULL so Fail path doesn't double-free
    goto Fail;
  }
