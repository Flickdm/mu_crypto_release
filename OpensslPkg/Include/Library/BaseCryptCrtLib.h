...
/**
  Macro that calls BaseCryptDebugPrint().

  Calls BaseCryptDebugPrint() passing in the debug error level and the
  format string and variable argument list.

  @param  Expression  Format string and variable argument list.
**/
#define BASECRYPT_DEBUG(Expression)   \
  do {                         \
    BaseCryptDebugPrint Expression; \
  } while (FALSE)
